...
model.CompartmentData.membranePot(:,end+1) = zeros(size(model.CompartmentData.membranePot,1),1);
model.CompartmentData.membranePot(end+1,:) = zeros(1,size(model.CompartmentData.membranePot,2));
model.CompartmentData.pH(end+1) = 5;  % https://doi.org/10.1046/j.1365-3040.1997.d01-67.x
...
model.CompartmentData.pH(strcmp(model.CompartmentData.compSymbolList,'m')) = 8;    % https://www.sciencedirect.com/science/article/pii/B9780123821638000219
...
% scale maximum uptake rate by the ratio of expected photon uptake and
...
%% Set oxygenation to carboxylation ratio
tmp_model = model;
phi_ml = 0.366;
phi_tol_ml = 1.1*0.087;

phi_fl = 0.329;
phi_tol_fl = 1.1*0.059;
...
% Guarantee for at least 50% of the the optimal RGR value
...
% logical vector with indices for bidirectional reactions
...
%% Read dry weight and net CO2 assimilation data
...
a_net_ml_wt = a_net_ml.mean_A(ismember(a_net_ml.Genotype, 'Col0'));
a_net_fl = readtable(fullfile(data_dir, 'A_fl.csv'));
a_net_fl_wt = a_net_fl.mean_A(ismember(a_net_fl.Genotype, 'Col0'));

a_net_nf_ids = {'NF_RBC_h', 'NF_RBO_h', 'NF_Tr_CO2m'};
    ...
    for t_idx = 1:numel(tp)
        ...
        for m_idx = 1:numel(mutants)
            ...
            if contains(l_cond(lc_idx), 'ml') || contains(l_cond(lc_idx), 'ml_fl')
                ...
                a_net_ratio = a_net_ml_wt / ...
                    a_net_ml.mean_A(strcmp(a_net_ml.Genotype, mutants{m_idx}));
            else
                biomass_ratio = mu_wt_fl / ...
                    mu_fl(strcmp(dw_fl.Properties.RowNames,mutants{m_idx}));
                a_net_ratio = a_net_fl_wt / ...
                    a_net_fl.mean_A(strcmp(a_net_fl.Genotype, mutants{m_idx}));
            end
            ...
            if m_idx == 1
                ...
                if lc_idx==1
                    ref_ml_growth = wt_bio_opt;
                    ref_ml_a_net = wt_a_net_opt;
                end
                
                % minimize the sum of absolute net fluxes at optimal
                % biomass predicted by TFA
                wt_tmodel.var_lb(wt_tmodel.f==1) = (1-1e-3)*wt_bio_opt;
            ...
            mut_tmodel.var_ub(mut_tmodel.f==1) = (1+1e-3) * wt_bio_opt / biomass_ratio;
            mut_tmodel.var_lb(mut_tmodel.f==1) = (1-1e-3) * wt_bio_opt / biomass_ratio;
            ...
            writetable(...
                ...
                [res_dir filesep l_cond{lc_idx} filesep 'diff_rxns_tfa_' mutants{m_idx} ...
            ...
            writetable(dir_tab,[res_dir filesep l_cond{lc_idx} filesep 'rxn_flexibility_' ...
                mutants{m_idx} ...
                '_' l_cond{lc_idx} ...
            ...
            writetable(...
                ...
                'minFlux_TVA_WT','maxFlux_TVA_WT','minFlux_TVA_Mut','maxFlux_TVA_Mut'},...
                'RowNames',model.rxns),...
                [res_dir filesep l_cond{lc_idx} filesep 'va_results_' mutants{m_idx} ...
                '_' l_cond{lc_idx} ...
                '_t_' num2str(tp(t_idx)) ...
                '_pHUB_' num2str(ph_ub,3) '.csv'],...
                'WriteRowNames', true)
            ...
            writetable(...
                ...
                [res_dir filesep l_cond{lc_idx} filesep 'met_conc_' mutants{m_idx} ...
                '_' l_cond{lc_idx} ...
                '_t_' num2str(tp(t_idx)) ...
                '_pHUB_' num2str(ph_ub,3) '.csv'],...
                'WriteRowNames',true)
            ...
            if m_idx == 1
                ...
                writematrix(...
                    fluxSamples_wt,...
                    [res_dir filesep l_cond{lc_idx} filesep 'flux_samples_Col-0_' ...
                    l_cond{lc_idx} ...
                    '_t_' num2str(tp(t_idx)) ...
                    '_pHUB_' num2str(ph_ub,3) '.csv'])
                ...
                writematrix(...
                    concSamples_wt,...
                    [res_dir filesep l_cond{lc_idx} filesep 'conc_samples_Col-0_' ...
                    l_cond{lc_idx} ...
                    '_t_' num2str(tp(t_idx)) ...
                    '_pHUB_' num2str(ph_ub,3) '.csv'])
            end
            ...
            writematrix(...
                fluxSamples_mut,...
                [res_dir filesep l_cond{lc_idx} filesep 'flux_samples_' mutants{m_idx} ...
                '_' l_cond{lc_idx} ...
                '_t_' num2str(tp(t_idx)) ...
                '_pHUB_' num2str(ph_ub,3) '.csv'])
            ...
            writematrix(...
                concSamples_mut,...
                [res_dir filesep l_cond{lc_idx} filesep 'conc_samples_' mutants{m_idx} ...
                '_' l_cond{lc_idx} ...
                '_t_' num2str(tp(t_idx)) ...
                '_pHUB_' num2str(ph_ub,3) '.csv'])
        end
