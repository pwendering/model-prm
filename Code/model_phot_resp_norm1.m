...
for t_idx = 1:numel(tp)
    ...
    for m_idx = 1:numel(mutants)
        ...
        % 1-norm minimization (v^mut - delta+ + delta- = v^wt)

        norm_1_lhs = zeros(N_COM, N_VARS);
        norm_1_lhs(:,COM_IDX) = eye(N_COM);
        norm_1_lhs(:,(CDIM+1):(CDIM+N_COM)) = -eye(N_COM);
        norm_1_lhs(:,(CDIM+N_COM+1):end) = eye(N_COM);
        
        norm_1_rhs = step_1_solution.x(COM_IDX);
              
        norm_1_sense = repelem('=', numel(COM_IDX),1);
        
        % steady state
        st_st_lhs = [model.S zeros(RDIM,2*N_COM)];
        ...
        bio_ratio_lhs = zeros(2,CDIM+2*N_COM);
        bio_ratio_lhs(1,find(model.c)) = -biomass_ratio;
        bio_ratio_lhs(2,find(model.c)) = biomass_ratio;
        
        bio_ratio_rhs = [-step_1_solution.f+1e-6; step_1_solution.f+1e-6];
        ...
        C_mut_lhs = zeros(2*sum(valid_idx),CDIM+2*N_COM);
        row_counter = 0;
        for i=1:numel(RXN_IDX)
            if valid_idx(i)
        ...
        problem.A = [st_st_lhs; bio_ratio_lhs; norm_1_lhs];
        problem.rhs = [st_st_rhs; bio_ratio_rhs; norm_1_rhs];
        problem.sense = [st_st_sense; bio_ratio_sense; norm_1_sense];
        
%         problem.A = [st_st_lhs; norm_1_lhs];
%         problem.rhs = [st_st_rhs;  norm_1_rhs];
%         problem.sense = [st_st_sense; norm_1_sense];
        
        problem.lb = [model.lb; zeros(2*N_COM,1)];
        problem.lb(findRxnIDs(model,ko_rxns{m_idx})) = 0;
        
        problem.ub = [model.ub; repelem(1000,2*N_COM,1)];
        problem.ub(findRxnIDs(model,ko_rxns{m_idx})) = 0;
        
%         problem.lb(find(model.c)) = step_1_solution.f / biomass_ratio - 1e-4;
%         problem.ub(find(model.c)) = step_1_solution.f / biomass_ratio + 1e-4;
        
        % objective
        problem.obj = zeros(N_VARS,1);
%         problem.obj(CDIM+1:end) = ones(2*N_COM,1);
        problem.obj(CDIM+1:end) = repmat(~ismember(common_rxns,'Bio_opt'),2,1);
        ...
        norm_1_solution = gurobi(problem,params);
        disp(norm_1_solution.status)
%         delta_plus = norm_1_solution.x(CDIM+1:CDIM+N_COM);
%         delta_minus = norm_1_solution.x(CDIM+N_COM+1:end);
%         disp([delta_plus(1:10) delta_minus(1:10)])
%         
%         if contains(norm_1_solution.status,'OPTIMAL')
%             % calculate ratio B_wt / B_mut
%             % B = C / (1-C)
%             B_wt_pfba = C_wt ./ (1-C_wt);
%             
%             C_mut_moma = calculateVBykE(model,norm_1_solution.x);
...
%         problem = struct;
%         
%         problem.A = [st_st_lhs; bio_ratio_lhs; norm_1_lhs; C_mut_lhs];
%         problem.rhs = [st_st_rhs; bio_ratio_rhs; norm_1_rhs; C_mut_rhs];
%         problem.sense = [st_st_sense; bio_ratio_sense; norm_1_sense; C_mut_sense];
%         
...
%                 )],...
%                 [res_dir filesep 'flux_dist_norm_1_' mutants{m_idx} ...
%                 't_' num2str(tp(t_idx)) ...
%                 '_' l_conds{l_idx} '.csv'],...
%                 'WriteRowNames', true...
%                 );
...
%                 })],...
%                 [res_dir filesep 'diff_rxns_norm_1_' mutants{m_idx} ...
%                 't_' num2str(tp(t_idx)) ...
%                 '_' l_conds{l_idx} '.csv']...
%                 );
%         end
