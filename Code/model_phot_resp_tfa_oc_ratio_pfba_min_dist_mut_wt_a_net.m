        ...
        for m_idx = 1:numel(mutants)
            ...
            if m_idx == 1
                ...
                if isempty(tfa_wt.x) && (ismember(l_cond(lc_idx), 'fl') || ismember(l_cond(lc_idx), 'fl_ml'))
                    ...
                    % Comment to following step: no reduction can be
                    % achieved
                    % minimize photon uptake at fixed growth rate
%                     wt_tmodel.f(:) = 0;
%                     wt_tmodel.f(strcmp(wt_tmodel.varNames,'F_Im_hnu')) = 1;
%                     wt_tmodel.objtype = 1;
%                     min_hnu = solveTFAmodelCplex(wt_tmodel);
%                     if min_hnu.val < ph_ub
%                         fprintf('Reduction of photon uptake from %.4g to %.4g (ML)\n',...
%                             ph_ub, min_hnu.val)
%                         ph_ub = (1+1e-6)*min_hnu.val;
%                         clear min_hnu
%                         % update photon uptake bound
%                         wt_tmodel.var_ub(strcmp(wt_tmodel.varNames,'F_Im_hnu')) = ph_ub;
%                         hnu_ratio_fl_ml = I_FL / I_ML;
%                         fprintf('Updating photon uptake for FL from %.4g to %.4g\n',...
%                             I_FL, ph_ub * hnu_ratio_fl_ml)
%                         I_FL = ph_ub * hnu_ratio_ml_fl;
%                     end
%                     
%                     % reset objective
%                     wt_tmodel.f(:) = 0;
