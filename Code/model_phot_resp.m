...
% fit Pool upper bound to guarantee the maximum measured CO2 assimilation rate
min_fun = @(UB)adjPoolToA(model,UB,A_max);
UB_fit = fminsearch(min_fun,0.1);
model.ub(strcmp(model.rxns,'prot_pool_exchange')) = UB_fit;
...
for t_idx = 1:numel(tp)
    ...
    for m_idx = 1:numel(mutants)
        ...
        %% Step 3 - calculate range for C^mut
        ...
        %% Step 4 - obtain final flux distributions for mutant and wild type
        ...
        norm_2_lhs = zeros(numel(COM_IDX),2*CDIM+numel(COM_IDX));
        norm_2_lhs(:,COM_IDX) = eye(numel(COM_IDX));
        norm_2_lhs(:,CDIM+COM_IDX) = -eye(numel(COM_IDX));
        norm_2_lhs(:,2*CDIM+1:end) = -eye(numel(COM_IDX));
        
        norm_2_rhs = zeros(numel(COM_IDX),1);
        
        norm_2_sense = repelem('=', numel(COM_IDX),1);
        
        % steady state
        st_st_lhs = [...
            model.S zeros(size(model.S)) zeros(RDIM,numel(COM_IDX));
            zeros(size(model.S)) model.S zeros(RDIM,numel(COM_IDX))...
            ];
        
        st_st_rhs = zeros(2*RDIM,1);
        
        st_st_sense = repelem('=',2*RDIM,1);
        
        % biomass ratio wt / mutant
        bio_ratio_lhs = zeros(1,2*CDIM+numel(COM_IDX));
        bio_ratio_lhs(repmat(model.c,2,1)==1) = [1 -biomass_ratio];
        
        bio_ratio_rhs = 0;
        
        bio_ratio_sense = '=';
        
        % photosynthesis rate constaints
        A_wt = A_ml.mean_A(strcmp(A_ml.Genotype,'Col0'));
        A_wt_sd = A_ml.sd_A(strcmp(A_ml.Genotype,'Col0'));
        A_mut = A_ml.mean_A(strcmp(A_ml.Genotype,mutants{m_idx}));
        A_mut_sd = A_ml.sd_A(strcmp(A_ml.Genotype,mutants{m_idx}));
        
        
        tmp_rxn_idx = findRxnIDs(model,{'arm_RBC_h' 'arm_RBO_h' 'Tr_CO2m_REV'});
        A_lhs = zeros(4,2*CDIM+numel(COM_IDX));
        A_lhs(1,tmp_rxn_idx) = [1 -0.5 -1];
        A_lhs(2,tmp_rxn_idx) = [-1 0.5 1];
        A_lhs(3,CDIM+tmp_rxn_idx) = [1 -0.5 -1];
        A_lhs(4,CDIM+tmp_rxn_idx) = [-1 0.5 1];
        
        A_rhs = [A_wt+A_wt_sd; -A_wt+A_wt_sd; A_mut+A_mut_sd; -A_mut+A_mut_sd];
        ...
        valid_idx = ...
            ...
            C_mut_max > 0 & ...
            C_mut_max > C_mut_min & ...
            abs(C_mut_max-C_mut_min) >= 0.2;
        
        C_mut_lhs = zeros(2*sum(valid_idx),2*CDIM);
        row_counter = 0;
        for i=1:numel(RXN_IDX)
            if valid_idx(i)
                ...
                for j=1:numel(tmp_rxn_enz_idx)
                    ...
                    C_mut_lhs(row_counter, CDIM + [RXN_IDX(i) tmp_draw_rxn_idx(j)]) = ...
        ...
        C_mut_lhs = [C_mut_lhs zeros(row_counter,numel(COM_IDX))];
        ...
        problem.A = [st_st_lhs; bio_ratio_lhs; A_lhs; norm_2_lhs; C_mut_lhs];
        problem.rhs = [st_st_rhs; bio_ratio_rhs; A_rhs; norm_2_rhs; C_mut_rhs];
        problem.sense = [st_st_sense; bio_ratio_sense; A_sense; norm_2_sense; C_mut_sense];
        
        problem.lb = [repmat(model.lb,2,1); -repelem(1000,numel(COM_IDX),1)];
        problem.lb(find(model.c)) = 0.9*step_1_solution.f;
        problem.lb(findRxnIDs(model,ko_rxns{m_idx})+CDIM) = 0;
        
        problem.ub = [repmat(model.ub,2,1); repelem(1000,numel(COM_IDX),1)];
        problem.ub(findRxnIDs(model,ko_rxns{m_idx})+CDIM) = 0;
        
        % objective
        problem.Q = sparse(2*CDIM+numel(COM_IDX),2*CDIM+numel(COM_IDX));
        problem.Q(2*CDIM+1:end, 2*CDIM+1:end) = speye(numel(COM_IDX));
        ...
        if ~contains(solution.status,'OPTIMAL')
            fprintf('Solver status: %s\n', solution.status)
        else
            
            % rank reactions by distance
            rxn_dist = solution.x(end-numel(COM_IDX)+1:end);
            ...
            writetable(...
                [cell2table(...
                [model.rxns(COM_IDX),...
                model.rxnNames(COM_IDX),...
                model.subSystems(COM_IDX)],...
                'VariableNames', {'RXN_ID', 'RXN_NAME', 'SUBSYSTEM'}...
                ),...
                array2table(...
                [rxn_dist_sorted,...
                solution.x(COM_IDX),...
                solution.x(COM_IDX+CDIM)],...
                'VariableNames', {'dist','wt_flux','mut_flux'}...
                )],...
                ['flux_dist_norm_2_' mutants{m_idx} ...
                't_' num2str(tp(t_idx)) ...
                '_' l_conds{l_idx} '.csv'],...
                'WriteRowNames', true...
                );
